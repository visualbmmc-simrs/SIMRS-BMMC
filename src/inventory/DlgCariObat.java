...
private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        if(TNoRw.getText().trim().equals("")){
            ...
            TCari.requestFocus();
                    ...
                    if(cekObatExpired()) {
            // Jika ada expired dan user memilih batalkan, hentikan proses
            return;
        }
...
private void buatcacheberiobat(){
    try{
        ...
        // ============================================================
        // BAGIAN 1: if(kenaikan > 0)
        // ============================================================
        if(kenaikan > 0){
            if(aktifkanbatch.equals("yes")){
                // ============================================================
                // QUERY GABUNGAN DENGAN PCare (aktifpcare = "yes")
                // ============================================================
                if(aktifpcare.equals("yes")){
                    sql = "SELECT * FROM (" +
                          "  SELECT " +
                          ...
                          "    jenis.nama, " +
                          "    databarang.kode_sat, " +
                          "    (data_batch.h_beli + (data_batch.h_beli * ?)) as harga, " +
                          ...
                          "    gudangbarang.stok, " +
                          "    data_batch." + hppfarmasi + " as dasar, " +
                          ...
                          "  INNER JOIN gudangbarang ON gudangbarang.kode_brng = data_batch.kode_brng " +
                          ...
                          "    '' as no_batch, " +
                          "    '' as no_faktur, " +
                          "    databarang.expire as tgl_kadaluarsa, " +
                          "    gudangbarang.stok, " +
                          "    databarang." + hppfarmasi + " as dasar, " +
                          "    databarang.kapasitas " +
                          "  FROM databarang " +
                          "  LEFT JOIN data_batch ON databarang.kode_brng = data_batch.kode_brng " +
                          "  INNER JOIN jenis ON databarang.kdjns = jenis.kdjns " +
                          "  INNER JOIN industrifarmasi ON industrifarmasi.kode_industri = databarang.kode_industri " +
                          "  INNER JOIN golongan_barang ON databarang.kode_golongan = golongan_barang.kode " +
                          "  INNER JOIN kategori_barang ON databarang.kode_kategori = kategori_barang.kode " +
                          ...
                          "    AND data_batch.kode_brng IS NULL " +
                    ...
                    psobat.setDouble(3, kenaikan);
                    psobat.setString(4, kdgudang.getText());
                
                // ============================================================
                // QUERY GABUNGAN TANPA PCare (aktifpcare = "no")
                // ============================================================
                } else {
                    sql = "SELECT * FROM (" +
                          "  SELECT " +
            ...
            // ============================================================
            // Bagian aktifkanbatch = "no" (TAMBAHKAN expire)
            // ============================================================
            } else {
                if(aktifpcare.equals("yes")){
                    sql="select databarang.kode_brng, databarang.nama_brng,jenis.nama, databarang.kode_sat,(databarang.h_beli+(databarang.h_beli*?)) as harga,"+
                        ...
                        " '' as no_batch, '' as no_faktur, databarang.expire as tgl_kadaluarsa "+
    ...
    /**
 * Method untuk mengecek apakah ada obat yang sudah expired
 * @return true jika ada obat expired DAN user memilih batalkan, false jika aman atau user lanjut
 */
private boolean cekObatExpired() {
    Date today = new Date();
    boolean adaExpired = false;
    StringBuilder daftarExpired = new StringBuilder();
    int jumlahExpired = 0;
    
    // Cek di tabel obat umum (tbObat)
    for(int i=0; i<tbObat.getRowCount(); i++) {
        if(Valid.SetAngka(tbObat.getValueAt(i, 1).toString()) > 0) {
            String tglKadaluarsa = "";
            try {
                tglKadaluarsa = tbObat.getValueAt(i, 18).toString(); // Kolom Kadaluarsa (index 18)
            } catch(Exception e) {
                tglKadaluarsa = "";
            }
            
            if(!tglKadaluarsa.equals("") && !tglKadaluarsa.equals("0000-00-00")) {
                try {
                    Date expiredDate = Valid.SetTgl2(tglKadaluarsa);
                    if(expiredDate.before(today)) {
                        adaExpired = true;
                        jumlahExpired++;
                        daftarExpired.append("\n• ").append(tbObat.getValueAt(i, 3).toString()) // Nama Obat
                                   .append(" (Batch: ").append(tbObat.getValueAt(i, 16).toString()) // No.Batch
                                   .append(", Exp: ").append(tglKadaluarsa).append(")");
                    }
                } catch(Exception e) {
    ...
    for(int i=0; i<tbDetailObatRacikan.getRowCount(); i++) {
        if(Valid.SetAngka(tbDetailObatRacikan.getValueAt(i, 10).toString()) > 0) {
            String tglKadaluarsa = "";
            try {
                tglKadaluarsa = tbDetailObatRacikan.getValueAt(i, 18).toString(); // Kolom Kadaluarsa (index 18)
            } catch(Exception e) {
                tglKadaluarsa = "";
            }
            ...
            if(!tglKadaluarsa.equals("") && !tglKadaluarsa.equals("0000-00-00")) {
                try {
                    ...
                    if(expiredDate.before(today)) {
                        adaExpired = true;
                        jumlahExpired++;
                        daftarExpired.append("\n• ").append(tbDetailObatRacikan.getValueAt(i, 2).toString()) // Nama Obat
                                   .append(" (Batch: ").append(tbDetailObatRacikan.getValueAt(i, 16).toString()) // No.Batch
                                   .append(", Exp: ").append(tglKadaluarsa).append(")");
                    }
                } catch(Exception e) {
                }
    ...
    if(adaExpired) {
        String pesan = "PERINGATAN OBAT KADALUARSA!\n\n" +
                       "Ditemukan " + jumlahExpired + " obat yang sudah melewati tanggal kadaluarsa:\n" +
                       daftarExpired.toString() +
                       "\n\nApakah Anda yakin ingin tetap memberikan obat-obatan ini?";
