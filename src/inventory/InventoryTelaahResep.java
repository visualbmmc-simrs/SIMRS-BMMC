    ...
    if(TabData.getSelectedIndex() == 0) {
        if(tabMode.getRowCount() == 0) {
            ...
            try {
                ...
                param.put("qrcode", NamaPetugas.getText() + " - " + akses.getnamars());
                
                String no_rawat = TNoRw.getText().trim();
                String no_resep = NoResep.getText().trim();
                
                String query = 
                    "SELECT " +
                    "    telaah_farmasi.no_resep, " +
                    "    DATE_FORMAT(resep_obat.tgl_perawatan, '%d-%m-%Y') AS tgl_perawatan, " +
                    "    DATE_FORMAT(resep_obat.jam, '%H:%i:%s') AS jam, " +
                    "    resep_obat.no_rawat, " +
                    ...
                    "    IFNULL(telaah_farmasi.resep_ket_identifikasi_pasien, '') AS resep_ket_identifikasi_pasien, " +
                    "    telaah_farmasi.resep_tepat_obat, " +
                    "    IFNULL(telaah_farmasi.resep_ket_tepat_obat, '') AS resep_ket_tepat_obat, " +
                    "    telaah_farmasi.resep_tepat_dosis, " +
                    "    IFNULL(telaah_farmasi.resep_ket_tepat_dosis, '') AS resep_ket_tepat_dosis, " +
                    "    telaah_farmasi.resep_tepat_cara_pemberian, " +
                    "    IFNULL(telaah_farmasi.resep_ket_tepat_cara_pemberian, '') AS resep_ket_tepat_cara_pemberian, " +
                    "    telaah_farmasi.resep_tepat_waktu_pemberian, " +
                    "    IFNULL(telaah_farmasi.resep_ket_tepat_waktu_pemberian, '') AS resep_ket_tepat_waktu_pemberian, " +
                    "    telaah_farmasi.resep_ada_tidak_duplikasi_obat, " +
                    "    IFNULL(telaah_farmasi.resep_ket_ada_tidak_duplikasi_obat, '') AS resep_ket_ada_tidak_duplikasi_obat, " +
                    "    telaah_farmasi.resep_interaksi_obat, " +
                    "    IFNULL(telaah_farmasi.resep_ket_interaksi_obat, '') AS resep_ket_interaksi_obat, " +
                    "    telaah_farmasi.resep_kontra_indikasi_obat, " +
                    "    IFNULL(telaah_farmasi.resep_ket_kontra_indikasi_obat, '') AS resep_ket_kontra_indikasi_obat, " +
                    ...
                    "    telaah_farmasi.nip, " +
                    "    petugas.nama AS petugas_farmasi, " +
"    GROUP_CONCAT(" +
"        CONCAT(" +
"            IFNULL(databarang.kode_brng, ''), ' | ', " +
"            IFNULL(databarang.nama_brng, ''), ' | ', " +
"            CAST(resep_dokter.jml AS CHAR), ' | ', " +
"            IFNULL(databarang.kode_sat, ''), ' | ', " +
"            IFNULL(resep_dokter.aturan_pakai, '')" +
"        ) SEPARATOR '\n'" +
"    ) AS daftar_obat " +
                    "FROM telaah_farmasi " +
                    "INNER JOIN resep_obat ON telaah_farmasi.no_resep = resep_obat.no_resep " +
                    ...
                    "LEFT JOIN resep_dokter ON resep_dokter.no_resep = resep_obat.no_resep " +
                    "LEFT JOIN databarang ON resep_dokter.kode_brng = databarang.kode_brng " +
                    "WHERE resep_obat.no_rawat = '" + no_rawat + "' " +
                    "AND telaah_farmasi.no_resep = '" + no_resep + "' " +
                    "GROUP BY telaah_farmasi.no_resep";
                
                Valid.MyReportqry(
                    "rptTelaahResepFarmasi.jasper",
                    "report",
                    "::[ FORM TELAAH RESEP & OBAT ]::",
                    query,
                    param
                );
