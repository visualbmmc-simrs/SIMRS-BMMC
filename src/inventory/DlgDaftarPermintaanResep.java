...
public class DlgDaftarPermintaanResep extends javax.swing.JDialog {
    ...
    private PreparedStatement pscetakracikan, pscetakracikandetail, pscetaknonracik;
    private PreparedStatement ps, ps2, ps3;  
    private ResultSet rscetakracikan, rscetakracikandetail, rscetaknonracik;
    private ResultSet rs, rs2, rs3;  
    private String rinciobat = "";
    ...
    private String filterJenisBayar = "";
    ...
    private javax.swing.JPanel panelPreviewKan;
    private javax.swing.JEditorPane txtPreviewResep;
    private javax.swing.JScrollPane scrollPreviewKan;
    ...
    private void initComponents() {
        ...
        DTPCari1.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "08-05-2026" }));
        ...
        DTPCari2.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "08-05-2026" }));
        ...
        cmbStatus1.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Semua", "BPJS" }));
        cmbStatus1.setName("cmbStatus1"); // NOI18N
        cmbStatus1.setPreferredSize(new java.awt.Dimension(130, 23));
        panelisi2.add(cmbStatus1);
        ...
        BtnObat23HariBPJS.setText("Kirim Klaim Obat BPJS");
    ...
    private void tbResepRalanMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbResepRalanMouseClicked
        if (tabMode.getRowCount() != 0) {
            try {
                getData();
                showSimplePreview(
                    NoResep, NoRawat, NoRM, Pasien, DokterPeresep, 
                    Status, TglPeresepan, JamPeresepan
                );
...
*/
        ...
        new SwingWorker<Void, Object[]>() {
            ...
            protected Void doInBackground() {
                try {
                    ...
                    filterJenisBayar = cmbStatus1.getSelectedItem().toString();
                    String filterBayarQuery = "";
                    if (!filterJenisBayar.equals("Semua")) {
                        filterBayarQuery = "and penjab.png_jawab = ? ";
                    }
                    ...
                    try {
                        ...
                        if (!filterJenisBayar.equals("Semua")) {
                            ps.setString(paramIndex++, filterJenisBayar);
                        }
                ...
                try {
                    ...
                    try {
                        ...
                        if (!semua) {
                            ...
                            ps.setString(paramIndex++, "%" + Kamar.getText().trim() + "%");
                            ps.setString(paramIndex++, "%" + TCari.getText() + "%");
                            ps.setString(paramIndex++, "%" + TCari.getText() + "%");
                            ps.setString(paramIndex++, "%" + TCari.getText() + "%");
                            ps.setString(paramIndex++, "%" + TCari.getText() + "%");
                    ...
                    // Query kedua untuk ranap_gabung
                    if (DEPOAKTIFOBAT.equals("")) {
                        ps = koneksi.prepareStatement("select resep_obat.no_resep,resep_obat.tgl_peresepan,resep_obat.jam_peresepan,"
                                + " resep_obat.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,resep_obat.kd_dokter,dokter.nm_dokter, "
                                + " if(resep_obat.tgl_perawatan='0000-00-00','Belum Terlayani','Sudah Terlayani') as status,"
   ...
   private void tampil7() {
    ...
    // ====== TAMBAHAN METHODE PREVIEW RESEP KHOLIS ======
    private void createPreviewPanelKan() {
    // Panel utama untuk preview
    panelPreviewKan = new javax.swing.JPanel(new BorderLayout());
    panelPreviewKan.setBackground(new java.awt.Color(255, 255, 255));
    panelPreviewKan.setBorder(javax.swing.BorderFactory.createTitledBorder(
    ...
    panelPreviewKan.setPreferredSize(new java.awt.Dimension(280, 0));
    panelPreviewKan.setMinimumSize(new java.awt.Dimension(280, 0));
    
    // Label header
    javax.swing.JLabel labelHeader = new javax.swing.JLabel("Klik baris data untuk preview", SwingConstants.CENTER);
    labelHeader.setFont(new java.awt.Font("Tahoma", 0, 11));
    labelHeader.setForeground(java.awt.Color.GRAY);
    panelPreviewKan.add(labelHeader, BorderLayout.NORTH);
    
    // Gunakan JEditorPane untuk mendukung HTML dan warna
    txtPreviewResep = new javax.swing.JEditorPane();
    txtPreviewResep.setEditable(false);
    txtPreviewResep.setContentType("text/html");
    txtPreviewResep.setBackground(new java.awt.Color(250, 250, 250));
    txtPreviewResep.setBorder(javax.swing.BorderFactory.createEmptyBorder(5, 5, 5, 5));
    txtPreviewResep.setText("<html><body style='font-family: Courier New; font-size: 12px; color: #333333;'>Pilih baris data resep untuk melihat preview...</body></html>");
    
    scrollPreviewKan = new javax.swing.JScrollPane(txtPreviewResep);
    scrollPreviewKan.setBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(200, 200, 200)));
    panelPreviewKan.add(scrollPreviewKan, BorderLayout.CENTER);
}

   private void addPreviewToRightSide() {
    
    // kholis panel preview
    createPreviewPanelKan();
    this.internalFrame1.add(panelPreviewKan, java.awt.BorderLayout.EAST);
    this.internalFrame1.revalidate();
    this.internalFrame1.repaint();
}

private void showSimplePreview(String noResep, String noRawat, String noRM, 
                               String pasien, String dokter, String status,
                               String tglResep, String jamResep) {
    if (txtPreviewResep == null) return;
    
    StringBuilder preview = new StringBuilder();
    // Gunakan CSS yang lebih spesifik dengan ukuran font yang lebih kecil
    preview.append("<html>");
    preview.append("<head>");
    preview.append("<style>");
    preview.append("body { font-family: 'Courier New', monospace; font-size: 10px; color: #333333; margin: 5px; }");
    preview.append("pre { font-family: 'Courier New', monospace; font-size: 10px; margin: 0; white-space: pre-wrap; }");
    preview.append(".apol { background-color: #00AA00; color: white; font-weight: bold; font-size: 8px; padding: 0px 4px; border-radius: 2px; }");
    preview.append("</style>");
    preview.append("</head>");
    preview.append("<body>");
    preview.append("<pre>");
    preview.append("═══════════════════════════════════════\n");
    preview.append("           PREVIEW RESEP\n");
    preview.append("═══════════════════════════════════════\n\n");
    
    preview.append("No.Resep    : ").append(noResep).append("\n");
    preview.append("Tgl Peresepan: ").append(tglResep).append("\n");
    preview.append("Jam Peresepan: ").append(jamResep).append("\n");
    preview.append("No.Rawat    : ").append(noRawat).append("\n");
    preview.append("No.RM       : ").append(noRM).append("\n");
    preview.append("Pasien      : ").append(pasien).append("\n");
    preview.append("Dokter      : ").append(dokter).append("\n");
    preview.append("Status      : ").append(status).append("\n");
    preview.append("───────────────────────────────────────\n");
    preview.append("OBAT:\n");
    
    // Ambil data obat dari database - KHUSUS RAWAT JALAN
    try {
        String query = "SELECT databarang.kode_brng, databarang.nama_brng, resep_dokter.jml, " +
                       "kodesatuan.satuan, resep_dokter.aturan_pakai, " +
                       "CASE WHEN maping_obat_apotek_bpjs.kode_brng IS NOT NULL THEN 1 ELSE 0 END AS is_apol " +
                       "FROM resep_obat " +
                       "INNER JOIN resep_dokter ON resep_obat.no_resep = resep_dokter.no_resep " +
                       "INNER JOIN databarang ON databarang.kode_brng = resep_dokter.kode_brng " +
                       "INNER JOIN kodesatuan ON kodesatuan.kode_sat = databarang.kode_sat " +
                       "LEFT JOIN maping_obat_apotek_bpjs ON databarang.kode_brng = maping_obat_apotek_bpjs.kode_brng " +
                       "WHERE resep_obat.no_resep = ? " +
                       "AND resep_dokter.aturan_pakai <> '' " +
                       "AND resep_obat.status = 'ralan'";
        
        PreparedStatement ps2 = koneksi.prepareStatement(query);
        ps2.setString(1, noResep);
        ...
        while (rs2.next()) {
            adaObat = true;
            String namaBrng = rs2.getString("nama_brng");
            String jml = rs2.getString("jml");
            String satuan = rs2.getString("satuan");
            String aturanPakai = rs2.getString("aturan_pakai");
            boolean isApol = rs2.getInt("is_apol") == 1;
            
            // Tampilkan nama obat
            preview.append(String.format("%d. %s", nomor, namaBrng));
            
            // PENANDA OBAT APOL - KHUSUS RAWAT JALAN
            if (isApol) {
                preview.append(" <span class='apol'>Obat Apol</span>");
            }
            preview.append("\n");
            preview.append(String.format("   Jml: %s %s\n", jml, satuan));
            preview.append(String.format("   Aturan: %s\n", aturanPakai));
            preview.append("\n");
            nomor++;
        }
        rs2.close();
        ...
        if (!adaObat) {
            preview.append("   (Tidak ada data obat untuk rawat jalan)\n");
        }
        
    } catch (Exception e) {
        preview.append("   Error: ").append(e.getMessage()).append("\n");
    }
    ...
    txtPreviewResep.setText(preview.toString());
    txtPreviewResep.setCaretPosition(0);
}
public void CetakEPrresKlaimPDF(String noresep, String norawat) {

    // ============================================================
    // 1. KASUS GABUNGAN (RACIKAN + NON-RACIK)
    // ============================================================
    if (Sequel.cariInteger("select COUNT(resep_dokter_racikan.no_resep) from resep_dokter_racikan where resep_dokter_racikan.no_resep='" + noresep + "'") > 0 
        && Sequel.cariInteger("select COUNT(resep_dokter.no_resep) from resep_dokter where resep_dokter.no_resep='" + noresep + "'") > 0) {
        ...
        paramgabung.put("noresep", noresep);
        paramgabung.put("tglresep", Sequel.cariIsi("select concat(resep_obat.tgl_peresepan, ',',resep_obat.jam_peresepan) from resep_obat where resep_obat.no_resep='" + noresep + "'"));
        paramgabung.put("dokterperesep", Sequel.cariIsi("select dokter.nm_dokter from dokter join resep_obat on dokter.kd_dokter=resep_obat.kd_dokter where resep_obat.no_resep='" + noresep + "'"));
        paramgabung.put("norm", Sequel.cariIsi("select reg_periksa.no_rkm_medis from reg_periksa where reg_periksa.no_rawat='" + norawat + "'"));
        paramgabung.put("penjamin", Sequel.cariIsi("select penjab.png_jawab from reg_periksa inner join penjab on reg_periksa.kd_pj=penjab.kd_pj where reg_periksa.no_rawat='" + norawat + "'"));
        paramgabung.put("namapasien", Sequel.cariIsi("select concat(pasien.nm_pasien,' / ',pasien.jk) from pasien where pasien.no_rkm_medis='" + Sequel.cariIsi("select reg_periksa.no_rkm_medis from reg_periksa where reg_periksa.no_rawat='" + norawat + "'") + "'"));
        paramgabung.put("alergipasien", Sequel.cariIsi("SELECT konseling_farmasi.riwayat_alergi FROM konseling_farmasi WHERE konseling_farmasi.no_rawat = '" + norawat + "'"));
        paramgabung.put("tgllahir", Sequel.cariIsi("select pasien.tgl_lahir from pasien where pasien.no_rkm_medis='" + Sequel.cariIsi("select reg_periksa.no_rkm_medis from reg_periksa where reg_periksa.no_rawat='" + norawat + "'") + "'") + " / " + Sequel.cariIsi("select concat(reg_periksa.umurdaftar,' ',reg_periksa.sttsumur) from reg_periksa where reg_periksa.no_rawat='" + norawat + "'"));
        paramgabung.put("alamat", Sequel.cariIsi("select concat(pasien.alamat,', ',kelurahan.nm_kel,', ',kecamatan.nm_kec,', ',kabupaten.nm_kab,', ',propinsi.nm_prop) as alamat from pasien inner join kelurahan on pasien.kd_kel=kelurahan.kd_kel inner join kecamatan on pasien.kd_kec=kecamatan.kd_kec "
                + "inner join kabupaten on pasien.kd_kab=kabupaten.kd_kab inner join propinsi on pasien.kd_prop=propinsi.kd_prop where pasien.no_rkm_medis='" + Sequel.cariIsi("select reg_periksa.no_rkm_medis from reg_periksa where reg_periksa.no_rawat='" + norawat + "'") + "'"));
        paramgabung.put("logo", Sequel.cariGambar("select logo from setting"));
        
        // ========== AMBIL FOTO UNTUK GABUNGAN ==========
        String fullUrlFotoGabung = "";
        try {
            ps = koneksi.prepareStatement("SELECT photo FROM bukti_penyerahan_resep_obat WHERE no_resep = ?");
            ps.setString(1, noresep);
            rs = ps.executeQuery();
            if (rs.next()) {
                String pathFoto = rs.getString("photo");
                if (pathFoto != null && !pathFoto.isEmpty()) {
                    pathFoto = pathFoto.trim();
                    pathFoto = pathFoto.replace("\\", "/");
                    fullUrlFotoGabung = "http://" + koneksiDB.HOST() + "/webapps/penyerahanresep/" + pathFoto;
                    System.out.println("URL Foto Gabung: " + fullUrlFotoGabung);
                }
            }
        } catch (Exception e) {
            System.out.println("Error ambil foto gabung: " + e);
        }
        paramgabung.put("pathFoto", fullUrlFotoGabung);

        Valid.MyReportqrypdfKlaim("rptBuktiResepGabung.jasper", "report", "2EPRES", 
                "select temporary_resep_gabung.no, temporary_resep_gabung.temp1,temporary_resep_gabung.temp2,temporary_resep_gabung.temp3,temporary_resep_gabung.temp4,temporary_resep_gabung.temp5 from temporary_resep_gabung where temporary_resep_gabung.temp6='" + noresep + "'", 
                paramgabung, "hasilkompilasifarmasi", norawat.replaceAll("/", ""));

        this.setCursor(Cursor.getDefaultCursor());
        
    // ============================================================
    // 2. KASUS HANYA RACIKAN
    // ============================================================
    } else if (Sequel.cariInteger("select COUNT(resep_dokter_racikan.no_resep) from resep_dokter_racikan where resep_dokter_racikan.no_resep='" + noresep + "'") > 0) {
        ...
        paramracik.put("noresep", tbResepRalan.getValueAt(tbResepRalan.getSelectedRow(), 0).toString());
        paramracik.put("tglresep", Sequel.cariIsi("select concat(resep_obat.tgl_peresepan, ',',resep_obat.jam_peresepan) from resep_obat where resep_obat.no_resep='" + noresep + "'"));
        paramracik.put("dokterperesep", Sequel.cariIsi("select dokter.nm_dokter from dokter join resep_obat on dokter.kd_dokter=resep_obat.kd_dokter where resep_obat.no_resep='" + noresep + "'"));
        paramracik.put("norm", Sequel.cariIsi("select reg_periksa.no_rkm_medis from reg_periksa where reg_periksa.no_rawat='" + norawat + "'"));
        paramracik.put("penjamin", Sequel.cariIsi("select penjab.png_jawab from reg_periksa inner join penjab on reg_periksa.kd_pj=penjab.kd_pj where reg_periksa.no_rawat='" + norawat + "'"));
        paramracik.put("namapasien", Sequel.cariIsi("select concat(pasien.nm_pasien,' / ',pasien.jk) from pasien where pasien.no_rkm_medis='" + Sequel.cariIsi("select reg_periksa.no_rkm_medis from reg_periksa where reg_periksa.no_rawat='" + norawat + "'") + "'"));
        paramracik.put("alergipasien", Sequel.cariIsi("SELECT konseling_farmasi.riwayat_alergi FROM konseling_farmasi WHERE konseling_farmasi.no_rawat = '" + norawat + "'"));
        ...
        // ========== AMBIL FOTO UNTUK RACIKAN ==========
        String fullUrlFotoRacik = "";
        try {
            ps = koneksi.prepareStatement("SELECT photo FROM bukti_penyerahan_resep_obat WHERE no_resep = ?");
            ps.setString(1, noresep);
            rs = ps.executeQuery();
            if (rs.next()) {
                ...
                if (pathFoto != null && !pathFoto.isEmpty()) {
                    pathFoto = pathFoto.trim();
                    pathFoto = pathFoto.replace("\\", "/");
                    fullUrlFotoRacik = "http://" + koneksiDB.HOST() + "/webapps/penyerahanresep/" + pathFoto;
                    System.out.println("URL Foto Racik: " + fullUrlFotoRacik);
                }
            }
        } catch (Exception e) {
            System.out.println("Error ambil foto racik: " + e);
        }
        paramracik.put("pathFoto", fullUrlFotoRacik);
        
        Valid.MyReportqrypdfKlaim("rptBuktiResepRacikan.jasper", "report", "2EPRESRACIK",
                "select temporary_resep_racikan.no, temporary_resep_racikan.temp1,temporary_resep_racikan.temp2,temporary_resep_racikan.temp3,temporary_resep_racikan.temp4 from temporary_resep_racikan where temporary_resep_racikan.temp5='" + noresep + "'", 
                paramracik, "hasilkompilasifarmasi", norawat.replaceAll("/", ""));
    ...
    // ============================================================
        ...
        param.put("emailrs", akses.getemailrs());
        param.put("noresep", noresep);
        param.put("tglresep", Sequel.cariIsi("select concat(resep_obat.tgl_peresepan, ',',resep_obat.jam_peresepan) from resep_obat where resep_obat.no_resep='" + noresep + "'"));
        param.put("dokterperesep", Sequel.cariIsi("select dokter.nm_dokter from dokter join resep_obat on dokter.kd_dokter=resep_obat.kd_dokter where resep_obat.no_resep='" + noresep + "'"));
        param.put("norm", Sequel.cariIsi("select reg_periksa.no_rkm_medis from reg_periksa where reg_periksa.no_rawat='" + norawat + "'"));
        param.put("penjamin", Sequel.cariIsi("select penjab.png_jawab from reg_periksa inner join penjab on reg_periksa.kd_pj=penjab.kd_pj where reg_periksa.no_rawat='" + norawat + "'"));
        param.put("namapasien", Sequel.cariIsi("select concat(pasien.nm_pasien,' / ',pasien.jk) from pasien where pasien.no_rkm_medis='" + Sequel.cariIsi("select reg_periksa.no_rkm_medis from reg_periksa where reg_periksa.no_rawat='" + norawat + "'") + "'"));
        param.put("alergipasien", Sequel.cariIsi("SELECT konseling_farmasi.riwayat_alergi FROM konseling_farmasi WHERE konseling_farmasi.no_rawat = '" + norawat + "'"));
        ...
        try {
            ...
            if (rs.next()) {
                ...
                if (pathFoto != null && !pathFoto.isEmpty()) {
                    pathFoto = pathFoto.trim();
                    pathFoto = pathFoto.replace("\\", "/");
                    fullUrlFoto = "http://" + koneksiDB.HOST() + "/webapps/penyerahanresep/" + pathFoto;
                    System.out.println("URL Foto Non-Racik: " + fullUrlFoto);
                }
            }
        } catch (Exception e) {
            System.out.println("Error ambil foto: " + e);
        }
        param.put("pathFoto", fullUrlFoto);

        Valid.MyReportqrypdfKlaim("rptBuktiResepNonRacik.jasper", "report", "2EPRESNR",
                "SELECT resep_dokter.kode_brng, databarang.nama_brng, resep_dokter.jml, resep_dokter.aturan_pakai, resep_dokter.no_resep FROM resep_obat INNER JOIN resep_dokter ON resep_obat.no_resep = resep_dokter.no_resep INNER JOIN databarang ON resep_dokter.kode_brng = databarang.kode_brng WHERE resep_obat.no_resep = '" + noresep + "'", 
                param, "hasilkompilasifarmasi", norawat.replaceAll("/", ""));
        this.setCursor(Cursor.getDefaultCursor());
    }
}
}
