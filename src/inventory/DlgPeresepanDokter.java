...
public final class DlgPeresepanDokter extends javax.swing.JDialog {
    ...
    private String[] cachedAturanMinum = null;
...
private void ppBersihkanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppBersihkanActionPerformed
    ...
    private void formWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowOpened
        ...
        SwingUtilities.invokeLater(() -> setupAturanPakaiDropdown());
    ...
    */
    ...
    /**
 * Method untuk mengambil data aturan minum dari database
 */
private String[] getAturanMinumList() {
    List<String> aturanList = new ArrayList<>();
    aturanList.add(""); // Tambahkan opsi kosong di awal
    try {
        Statement st = koneksi.createStatement();
        ResultSet rs = st.executeQuery("SELECT aturan FROM aturan_minum ORDER BY id");
        while (rs.next()) {
            String aturan = rs.getString("aturan");
        ...
        System.out.println("Error mengambil data aturan minum: " + e);
    }
    return aturanList.toArray(new String[0]);
}

/**
 * Method untuk mendapatkan data aturan minum yang sudah di-cache
 */
private String[] getCachedAturanMinum() {
    if (cachedAturanMinum == null) {
        cachedAturanMinum = getAturanMinumList();
    }
    return cachedAturanMinum;
}

/**
 * Method untuk setup dropdown aturan pakai pada tabel
 */
private void setupAturanPakaiDropdown() {
    try {
        // Setup dropdown untuk tabel Umum (kolom Aturan Pakai index 2)
        if (tbResep.getColumnCount() > 2) {
            String[] aturanList = getCachedAturanMinum();
            if (aturanList != null && aturanList.length > 0) {
                JComboBox<String> cmbAturanPakai = new JComboBox<>(aturanList);
                cmbAturanPakai.setEditable(true);
                DefaultCellEditor aturanPakaiEditor = new DefaultCellEditor(cmbAturanPakai);
                tbResep.getColumnModel().getColumn(2).setCellEditor(aturanPakaiEditor);
            }
        }
        
        // Setup dropdown untuk tabel Racikan (kolom Aturan Pakai index 5)
        if (tbObatResepRacikan.getColumnCount() > 5) {
            String[] aturanList = getCachedAturanMinum();
            if (aturanList != null && aturanList.length > 0) {
                JComboBox<String> cmbAturanPakaiRacikan = new JComboBox<>(aturanList);
                cmbAturanPakaiRacikan.setEditable(true);
                DefaultCellEditor aturanPakaiEditorRacikan = new DefaultCellEditor(cmbAturanPakaiRacikan);
                tbObatResepRacikan.getColumnModel().getColumn(5).setCellEditor(aturanPakaiEditorRacikan);
            }
        }
    } catch (Exception e) {
        System.out.println("Error setup dropdown aturan pakai: " + e);
    }
}
