...
private void ChkInputActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ChkInputActionPerformed
  isForm();                
    ...
    */
    ...
    private void simpanuntukcetak(String p1, String p2, String p3, String p4) {
    ...
    public void tampilBerkasKlaimObat(String norawat, String norm) {
        this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        ...
        try {
            ps = koneksi.prepareStatement("select resep_obat.no_resep,resep_obat.tgl_perawatan,resep_obat.jam,"
                    ...
                    + " resep_obat.no_rawat='" + norawat + "' order by resep_obat.tgl_perawatan,resep_obat.jam");
            try {
                rs = ps.executeQuery();
                jumlahtotal = 0;
                while (rs.next()) {

                    simpanuntukcetak(rs.getString("no_resep"), rs.getString("tgl_perawatan") + " " + rs.getString("jam"),
                            rs.getString("no_rawat") + " " + rs.getString("no_rkm_medis") + " " + rs.getString("nm_pasien"),
                            rs.getString("nm_dokter"));

                    simpanuntukcetak("", "Nama Obat", "Jumlah x Harga + Embalase + Tuslah = Total", "Aturan Pakai");
                    ...
                    try {
                        ...
                        total = 0;
                        while (rs2.next()) {
                            simpanuntukcetak("", rs2.getString("nama_brng"), rs2.getString("jml") + "  x  " + Valid.SetAngka(rs2.getDouble("biaya_obat"))
                                    + " + " + Valid.SetAngka(rs2.getDouble("embalase")) + " + " + Valid.SetAngka(rs2.getDouble("tuslah")) + " = " + Valid.SetAngka(rs2.getDouble("total")),
                    ...
                    try {
                        ...
                        rsracikan = psracikan.executeQuery();
                        while (rsracikan.next()) {
                            simpanuntukcetak("", rsracikan.getString("no_racik") + ". " + rsracikan.getString("nama_racik"),
                                    rsracikan.getString("jml_dr") + " " + rsracikan.getString("metode") + ", Keterangan : " + rsracikan.getString("keterangan"),
                                    rsracikan.getString("aturan_pakai"));
                            ps2 = koneksi.prepareStatement(
                    ...
                    if (total > 0) {
                        simpanuntukcetak("", "", "Total Biaya Resep = " + Valid.SetAngka(total), "");
                    }
                }
                rs.last();
                if (rs.getRow() > 0) {
                    simpanuntukcetak(">>", "Jumlah Total Biaya Resep", Valid.SetAngka(jumlahtotal), "");
                }
            } catch (Exception ex) {
                System.out.println("Notifikasi : " + ex);
            } finally {
            ...
            Valid.MyReport2toPDFFiles("rptResep.jasper", "report", "3BILLING", param, "hasilkompilasifarmasi", norawat.replaceAll("/", ""));
            this.setCursor(Cursor.getDefaultCursor());
        } catch (SQLException e) {
            System.out.println("Notifikasi : " + e);
        }
