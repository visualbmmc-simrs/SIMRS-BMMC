...
public class DlgPasien extends javax.swing.JDialog {
    ...
    private SatuSehatCekNIK satusehat = new SatuSehatCekNIK();
    ...
    private void initComponents() {
        ...
        DTPLahir.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "31-08-2026" }));
        ...
        DTPDaftar.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "31-08-2026" }));
        ...
        BtnRefreshPhoto1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/kemkes.png"))); // NOI18N
        BtnRefreshPhoto1.setMnemonic('U');
        BtnRefreshPhoto1.setText("Cek IHS");
        ...
        FormInput.add(BtnRefreshPhoto1);
        BtnRefreshPhoto1.setBounds(880, 130, 90, 30);
                    ...
                    if (Sequel.menyimpantf2("pasien", "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", "No.Rekam Medis Pasien", 36, new String[]{
                        ...
                        if (Sequel.menyimpantf2("pasien", "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", "No.Rekam Medis Pasien", 36, new String[]{
                            ...
                            if (Sequel.menyimpantf("pasien", "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", "No.Rekam Medis Pasien", 36, new String[]{
                                ...
                                cekAtauBuatIhsLatar(TKtp.getText());
                                emptTeks();
                            } else {
                                TNm.requestFocus();
                                autoNomor();
...
private void KabupatenMouseMoved(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_KabupatenMouseMoved
    ...
    private void BtnRefreshPhoto1ActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        // Jika ditemukan tapi tanpa IHS (provisional)
        ...
        // Jika tidak ditemukan, tanya user apakah mau daftar
        ...
        // Proses pendaftaran ke Satu Sehat
        SatuSehatCekNIK.HasilIHS daftar;
        this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        try {
            daftar = satusehat.daftarkanPasien(nik);
        } finally {
            this.setCursor(Cursor.getDefaultCursor());
        }
    ...
    private void cekAtauBuatIhsLatar(final String nik) {
        final String bersih = (nik == null) ? "" : nik.trim();

        if (!bersih.matches("\\d{16}")) {
            return;
        }
        ...
        Thread pekerja = new Thread(new Runnable() {
            ...
            public void run() {
                SatuSehatCekNIK.HasilIHS hasil;
                try {
                    hasil = satusehat.cekIHSAtauBuat(bersih);
                } catch (Throwable e) {
                    System.out.println("Notifikasi IHS pasien " + bersih + " : " + e);
                    return;
                }
                System.out.println("IHS pasien " + bersih + " : " + hasil.status + " - " + hasil.pesan);
            }
        }, "ihs-pasien-" + bersih);
