...
public final class RMCatatanADIMEGizi extends javax.swing.JDialog {
    ...
    public RMCatatanADIMEGizi(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{
            "Pilih", "No.Rawat", "No.R.M.", "Nama Pasien", "Umur", "JK", "Tanggal", "Asesmen", "Diagnosis", "Intervensi", "Monitoring", "Evaluasi", "Instruksi", "NIP", "Petugas"
        }) {
            @Override
    ...
    private void initComponents() {
        ...
        MnBatalkanPilihan = new javax.swing.JMenuItem();
        ...
        MnPilihSemua.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/accept.png")));
        MnPilihSemua.setText("Pilih Semua");
        MnPilihSemua.setName("MnPilihSemua");
        MnPilihSemua.setPreferredSize(new java.awt.Dimension(240, 26));
        ...
        MnBatalkanPilihan.setBackground(new java.awt.Color(255, 255, 254));
        MnBatalkanPilihan.setFont(new java.awt.Font("Tahoma", 0, 11));
        MnBatalkanPilihan.setForeground(new java.awt.Color(50, 50, 50));
        MnBatalkanPilihan.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/Cancel-2-16x16.png")));
        MnBatalkanPilihan.setText("Batalkan Pilihan");
        MnBatalkanPilihan.setName("MnBatalkanPilihan");
        MnBatalkanPilihan.setPreferredSize(new java.awt.Dimension(240, 26));
        MnBatalkanPilihan.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                MnBatalkanPilihanActionPerformed(evt);
            }
        });
        jPopupMenu1.add(MnBatalkanPilihan);
        ...
        jComboBox1.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Pilih Etiket", "Etiket pagi", "Etiket siang", "Etiket sore" }));
        ...
        Tanggal.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "04-06-2026" }));
    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
        ...
        for (int row = 0; row < tabMode.getRowCount(); row++) {
            Boolean isSelected = (Boolean) tabMode.getValueAt(row, 0);
        ...
        if (adaPilihan || tbObat.getSelectedRow() > -1) {
            if (akses.getkode().equals("Admin Utama")) {
                hapus();
            } else {
                if (tbObat.getSelectedRow() > -1) {
                    ...
                    JOptionPane.showMessageDialog(null, "Silahkan klik/highlight 1 baris data terlebih dahulu untuk validasi petugas..!!");
                }
            }
        } else {
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            boolean adaPilihan = false;
            StringBuilder noRawatList = new StringBuilder();
            
            for (int row = 0; row < tabMode.getRowCount(); row++) {
                Boolean isSelected = (Boolean) tabMode.getValueAt(row, 0);
                if (isSelected != null && isSelected) {
                    adaPilihan = true;
                    String noRawat = tabMode.getValueAt(row, 1).toString();
                    if (noRawatList.length() > 0) {
                        noRawatList.append("','");
                    }
                    noRawatList.append(noRawat);
                }
            }
            
            String filterQuery = "";
            if (adaPilihan) {
                filterQuery = "and reg_periksa.no_rawat in ('" + noRawatList.toString() + "') ";
            }
            ...
            if (TCari.getText().equals("")) {
                Valid.MyReportqry("rptDataCatatanADIMEGizi.jasper", "report", "::[ Data Catatan ADIME Gizi Pasien ]::",
                        ...
                        + "catatan_adime_gizi.tanggal between '" + Valid.SetTgl(DTPCari1.getSelectedItem() + "") + " 00:00:00' and '" + Valid.SetTgl(DTPCari2.getSelectedItem() + "") + " 23:59:59' "
                        + filterQuery
                        + "order by catatan_adime_gizi.tanggal ", param);
            } else {
                Valid.MyReportqry("rptDataCatatanADIMEGizi.jasper", "report", "::[ Data Catatan ADIME Gizi Pasien ]::",
                        "select reg_periksa.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,reg_periksa.umurdaftar,reg_periksa.sttsumur,"
                        + "pasien.jk,catatan_adime_gizi.tanggal,catatan_adime_gizi.asesmen,catatan_adime_gizi.diagnosis,"
                        ...
                        + "catatan_adime_gizi.tanggal between '" + Valid.SetTgl(DTPCari1.getSelectedItem() + "") + " 00:00:00' and '" + Valid.SetTgl(DTPCari2.getSelectedItem() + "") + " 23:59:59' "
                        + filterQuery
                        + "and (reg_periksa.no_rawat like '%" + TCari.getText().trim() + "%' or pasien.no_rkm_medis like '%" + TCari.getText().trim() + "%' or pasien.nm_pasien like '%" + TCari.getText().trim() + "%' or "
    ...
    private void MnCatatanADIMEActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnCatatanADIMEActionPerformed
        if (tbObat.getSelectedRow() > -1) {
            ...
            Valid.MyReportqry("rptFormulirCatatanADIMEGizi.jasper", "report", "::[ Formulir Catatan ADIME Gizi Pasien ]::",
                    ...
                    + "from catatan_adime_gizi inner join reg_periksa on catatan_adime_gizi.no_rawat=reg_periksa.no_rawat "
                    + "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                    + "inner join petugas on catatan_adime_gizi.nip=petugas.nip where reg_periksa.no_rawat='" + tbObat.getValueAt(tbObat.getSelectedRow(), 1).toString() + "'", param);
        }
    ...
    private void MnBatalkanPilihanActionPerformed(java.awt.event.ActionEvent evt) {
        for (int row = 0; row < tabMode.getRowCount(); row++) {
    ...
    private void jComboBox1ActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        if (selectedItem.equals("Pilih Etiket")) {
            return;
        }

        boolean adaCentang = false;
        for (int row = 0; row < tabMode.getRowCount(); row++) {
            Boolean isSelected = (Boolean) tabMode.getValueAt(row, 0);
            if (isSelected != null && isSelected) {
                adaCentang = true;
        ...
        if (adaCentang || tbObat.getSelectedRow() > -1) {
            if (selectedItem.equals("Etiket pagi")) {
                cetakEtiketMakan(null, "Pagi");
            } else if (selectedItem.equals("Etiket siang")) {
                cetakEtiketMakan(null, "Siang");
            } else if (selectedItem.equals("Etiket sore")) {
                cetakEtiketMakan(null, "Sore");
            }
        } else {
            JOptionPane.showMessageDialog(rootPane, "Silahkan pilih/centang data pasien terlebih dahulu..!!");
        }
    ...
    private javax.swing.JMenuItem MnBatalkanPilihan;
    ...
    private String getKelasKamar(String noRawat) {
        String kamar = "-";
        PreparedStatement psKamar = null;
        ResultSet rsKamar = null;

        try {
            psKamar = koneksi.prepareStatement(
                    "SELECT kamar_inap.kd_kamar, kamar.kelas, bangsal.nm_bangsal "
                    + "FROM kamar_inap "
                    + "INNER JOIN kamar ON kamar_inap.kd_kamar = kamar.kd_kamar "
                    + "INNER JOIN bangsal ON kamar.kd_bangsal = bangsal.kd_bangsal "
                    + "WHERE kamar_inap.no_rawat = ? "
                    + "AND (kamar_inap.tgl_keluar = '0000-00-00' OR kamar_inap.tgl_keluar IS NULL) "
                    + "ORDER BY kamar_inap.tgl_masuk DESC, kamar_inap.jam_masuk DESC LIMIT 1"
            );
            psKamar.setString(1, noRawat);
            rsKamar = psKamar.executeQuery();

            if (rsKamar.next()) {
                String kelas = rsKamar.getString("kelas");
                String nmBangsal = rsKamar.getString("nm_bangsal");

                if (kelas != null && !kelas.equals("")) {
                    kamar = kelas;
                    if (nmBangsal != null && !nmBangsal.equals("")) {
                        kamar = kamar + " - " + nmBangsal;
                    }
                } else {
                    kamar = nmBangsal != null ? nmBangsal : "-";
                }
            } else {
                psKamar = koneksi.prepareStatement(
                        "SELECT kamar_inap.kd_kamar, kamar.kelas, bangsal.nm_bangsal "
            ...
            System.out.println("Error getKelasKamar: " + e);
            kamar = "-";
        } finally {
            try {
                if (rsKamar != null) {
    ...
    private void cetakEtiketMakan(java.awt.event.ActionEvent evt, String waktuMakan) {
        boolean adaCentang = false;
        StringBuilder noRawatList = new StringBuilder();
        ...
        if (adaCentang || tbObat.getSelectedRow() > -1) {
            if (!adaCentang && tbObat.getSelectedRow() > -1) {
                noRawatList.append(tbObat.getValueAt(tbObat.getSelectedRow(), 1).toString());
            }
            ...
            try {
                ...
                param.put("emailrs", akses.getemailrs());
                param.put("logo", Sequel.cariGambar("select setting.logo from setting"));
                param.put("waktu_makan", waktuMakan);
                param.put("tanggal_cetak", new Date());

                Valid.MyReportqry("rptEtiketMakanPasien.jasper", "report",
                        "::[ Etiket Makan Pasien - " + waktuMakan + " ]::",
                        "SELECT pasien.nm_pasien, pasien.no_rkm_medis, "
                        + "IFNULL(("
                        + "  SELECT CONCAT(kamar.kelas, ' - ', bangsal.nm_bangsal) "
                        + "  FROM kamar_inap INNER JOIN kamar ON kamar_inap.kd_kamar = kamar.kd_kamar "
                        + "  INNER JOIN bangsal ON kamar.kd_bangsal = bangsal.kd_bangsal "
                        + "  WHERE kamar_inap.no_rawat = reg_periksa.no_rawat "
                        + "  AND (kamar_inap.tgl_keluar = '0000-00-00' OR kamar_inap.tgl_keluar IS NULL) LIMIT 1"
                        + "), '-') AS kelas_kamar, "
                        + "catatan_adime_gizi.instruksi AS diet "
                        + "FROM catatan_adime_gizi "
                        + "INNER JOIN reg_periksa ON catatan_adime_gizi.no_rawat = reg_periksa.no_rawat "
                        + "INNER JOIN pasien ON reg_periksa.no_rkm_medis = pasien.no_rkm_medis "
                        + "WHERE reg_periksa.no_rawat IN ('" + noRawatList.toString() + "') ",
                        param);

            } catch (Exception e) {
                System.out.println("Error cetakEtiketMakan: " + e);
                JOptionPane.showMessageDialog(null, "Gagal mencetak etiket: " + e.getMessage());
            }
        } else {
            JOptionPane.showMessageDialog(rootPane, "Silahkan pilih/centang data pasien terlebih dahulu..!!");
        }
    ...
    private void getData() {
        if (tbObat.getSelectedRow() != -1) {
            ...
            Asesmen.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 7).toString());
            ...
            Monitoring.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 10).toString());
            Evaluasi.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 11).toString());
            Instruksi.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 12).toString());
        }
    }
    ...
    private void ganti() {
        if (Sequel.mengedittf("catatan_adime_gizi", "tanggal=? and no_rawat=?", "no_rawat=?,tanggal=?,asesmen=?,diagnosis=?,intervensi=?,monitoring=?,evaluasi=?,instruksi=?,nip=?", 11, new String[]{
            ...
            Intervensi.getText(), Monitoring.getText(), Evaluasi.getText(), Instruksi.getText(), KdPetugas.getText(), tbObat.getValueAt(tbObat.getSelectedRow(), 6).toString(),
            ...
            tbObat.setValueAt(Asesmen.getText(), tbObat.getSelectedRow(), 7);
            tbObat.setValueAt(Diagnosis.getText(), tbObat.getSelectedRow(), 8);
            tbObat.setValueAt(Intervensi.getText(), tbObat.getSelectedRow(), 9);
            tbObat.setValueAt(Monitoring.getText(), tbObat.getSelectedRow(), 10);
            tbObat.setValueAt(Evaluasi.getText(), tbObat.getSelectedRow(), 11);
            tbObat.setValueAt(Instruksi.getText(), tbObat.getSelectedRow(), 12);
    ...
    private void hapus() {
        if (Sequel.queryu2tf("delete from catatan_adime_gizi where tanggal=? and no_rawat=?", 2, new String[]{
            tbObat.getValueAt(tbObat.getSelectedRow(), 6).toString(), tbObat.getValueAt(tbObat.getSelectedRow(), 1).toString()
        }) == true) {
