...
public final class ApotekBPJSResepObat extends javax.swing.JDialog {
    ...
    private String selectedNoSEP = "";
    private String selectedNoSEPApotek = "";
    private String selectedNoRawat = "";
    private String selectedNoRM = "";
    private String selectedNoResep = "";
    // ============ END TAMBAHAN VARIABEL ============
    ...
    public ApotekBPJSResepObat(java.awt.Frame parent, boolean modal) {
        ...
        // ============================================================
        // MODIFIKASI: Ubah jLabel6 menjadi Total Pasien
        // ============================================================
        jLabel6.setText("Total :");
        jLabel6.setPreferredSize(new Dimension(55, 23));
        jLabel6.setForeground(new java.awt.Color(0, 0, 200));
        // ============================================================

        // ============================================================
        // TAMBAH: Klik Kanan (hanya menu PDF, tanpa refresh)
        // ============================================================
        tbKamar.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                if (SwingUtilities.isRightMouseButton(e)) {
                    int row = tbKamar.rowAtPoint(e.getPoint());
                    if (row != -1) {
                        tbKamar.setRowSelectionInterval(row, row);
                        showPopupMenu(e);
                    }
                }
            }
    ...
    // ============================================================
    // METHOD POPUP MENU (HANYA PDF)
    // ============================================================
    private void showPopupMenu(MouseEvent e) {
        JPopupMenu popupMenu = new JPopupMenu();

        JMenuItem menuBerkasKlaim = new JMenuItem("Berkas Klaim (PDF)");
        menuBerkasKlaim.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/pdf.png")));
        menuBerkasKlaim.addActionListener(evt -> {
            generateBerkasKlaim();
        });
        popupMenu.add(menuBerkasKlaim);

        popupMenu.show(tbKamar, e.getX(), e.getY());
    }

    // ============================================================
    // METHOD GENERATE BERKAS KLAIM
    // ============================================================
    private void generateBerkasKlaim() {
        if (tbKamar.getSelectedRow() == -1) {
            JOptionPane.showMessageDialog(rootPane,
                    "Maaf, silahkan pilih data resep terlebih dahulu!",
                    "Peringatan",
        ...
        try {
            selectedNoSEPApotek = tbKamar.getValueAt(tbKamar.getSelectedRow(), 0).toString();
            selectedNoSEP = tbKamar.getValueAt(tbKamar.getSelectedRow(), 1).toString();
            selectedNoResep = tbKamar.getValueAt(tbKamar.getSelectedRow(), 2).toString();

            selectedNoRawat = getNoRawatFromSEP(selectedNoSEP);
            selectedNoRM = getNoRMFromSEP(selectedNoSEP);

            if (selectedNoRawat.isEmpty()) {
                JOptionPane.showMessageDialog(rootPane,
                        "Tidak ditemukan data rawat untuk SEP: " + selectedNoSEP,
                        "Error",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }

            gabungberkasobat(selectedNoRawat, selectedNoSEP);

        } catch (Exception ex) {
            System.out.println("Error generate berkas klaim: " + ex);
            JOptionPane.showMessageDialog(rootPane,
    ...
    // ============================================================
    private String getNoRawatFromSEP(String noSEP) {
        String noRawat = "";
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            ps = koneksi.prepareStatement(
                    "SELECT no_rawat FROM bridging_sep WHERE no_sep = ?"
            );
            ...
            System.out.println("Error getNoRawatFromSEP: " + e);
        } finally {
            try {
                if (rs != null) {
                    rs.close();
    ...
    private String getNoRMFromSEP(String noSEP) {
        String noRM = "";
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            ps = koneksi.prepareStatement(
                    "SELECT nomr FROM bridging_sep WHERE no_sep = ?"
            );
            ps.setString(1, noSEP);
            rs = ps.executeQuery();
            if (rs.next()) {
                noRM = rs.getString("nomr");
            }
        } catch (Exception e) {
            System.out.println("Error getNoRMFromSEP: " + e);
        } finally {
            try {
                if (rs != null) {
                    rs.close();
    ...
    // ============================================================
    // FUNGSI PDF (PERSIS SEPERTI CONTOH)
    // ============================================================
    private void gabungberkasobat(String norawat, String nosep) {
        this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        Properties systemProp = System.getProperties();
        String currentDir = systemProp.getProperty("user.dir");
        String folderPath = currentDir + "/hasilkompilasifarmasi/";
        String outputPath = currentDir + "/hasilkompilasifarmasi/HASILGABUNG" + nosep.replaceAll("/", "") + ".pdf";
        ...
        if (Sequel.cariInteger("select count(resep_obat.no_rawat) from resep_obat where resep_obat.no_rawat='" + norawat + "'") > 0) {
            try {
                inventory.DlgResepObat billingresep = new inventory.DlgResepObat(null, false);
                billingresep.tampilBerkasKlaimObat(norawat, selectedNoRM);
            } catch (Exception e) {
                System.out.println("Error generate resep obat: " + e);
            }
        }

        if (Sequel.cariInteger("select count(resep_obat.no_rawat) from resep_obat where resep_obat.no_rawat='" + norawat + "'") > 0) {
            try {
                PreparedStatement pscetakepress = koneksi.prepareStatement(
                        "select resep_obat.no_resep from resep_obat where resep_obat.no_rawat='" + norawat + "' order by resep_obat.tgl_perawatan desc, resep_obat.jam desc limit 1"
                );
                ResultSet rscetakepress = pscetakepress.executeQuery();
                if (rscetakepress.next()) {
                    inventory.DlgDaftarPermintaanResep epress = new inventory.DlgDaftarPermintaanResep(null, false);
                    epress.CetakEPrresKlaimPDF(rscetakepress.getString("no_resep"), norawat);
                }
                rscetakepress.close();
                pscetakepress.close();
            } catch (Exception e) {
                System.out.println("Error generate e-prescription: " + e);
            }
        }

        btnSEPResumePDFFarmasi(norawat, nosep);

        if (Sequel.cariInteger("select count(no_rawat) from periksa_lab where no_rawat='" + norawat + "'") > 0) {
            try {
                simrskhanza.DlgCariPeriksaLab hasillab = new simrskhanza.DlgCariPeriksaLab(null, false);
                hasillab.HasilLabPDFKlaimBPJSKompilasiFarmasi(norawat, selectedNoRM);
            } catch (Exception e) {
                System.out.println("Error generate hasil lab: " + e);
            }
        }
    ...
    private void mergePDFsFromFolder(String folderPath, String outputPath, String nomorrawat) {
        ...
        if (files != null) {
            ...
            for (File file : files) {
                if (file.isFile() && file.getName().contains(nomorrawat) && file.getName().endsWith(".pdf")) {
                    try {
    ...
    private void btnSEPResumePDFFarmasi(String norawat, String nosep) {
        if (norawat.equals("")) {
            JOptionPane.showMessageDialog(rootPane, "Maaf, silahkan pilih pasien terlebih dahulu");
            return;
        }

        String statusRawat = Sequel.cariIsi("select reg_periksa.status_lanjut from reg_periksa where reg_periksa.no_rawat='" + norawat + "'");

        if (statusRawat.equals("Ralan")) {
            String nomorsep = Sequel.cariIsi("select bridging_sep.no_sep from bridging_sep where bridging_sep.no_rawat='" + norawat + "' and bridging_sep.jnspelayanan='2'");
            String kodedokterregis = Sequel.cariIsi("select reg_periksa.kd_dokter from reg_periksa where reg_periksa.no_rawat='" + norawat + "'");
            ...
            String diagnosa = Sequel.cariIsi(
                    "select nm_penyakit from diagnosa_pasien "
                    + "join penyakit on diagnosa_pasien.kd_penyakit=penyakit.kd_penyakit "
                    + "where prioritas='1' and no_rawat='" + norawat + "'"
            );
            param.put("diagnosautama", diagnosa != null ? diagnosa : "");
            param.put("uraian", "");
            param.put("prosedurutama", "");
            param.put("finger", "Dikeluarkan di " + akses.getnamars());

            Valid.MyReportPDFKlaim("rptBridgingSEP4Resume.jasper", "report", "1SEP", param, "hasilkompilasifarmasi", norawat.replaceAll("/", ""));
        } else if (statusRawat.equals("Ranap")) {
            String nomorsep = Sequel.cariIsi("select bridging_sep.no_sep from bridging_sep where bridging_sep.jnspelayanan='1' and bridging_sep.no_rawat='" + norawat + "'");
            ...
            Valid.MyReportPDFKlaim("rptBridgingSEP.jasper", "report", "1SEP", param, "hasilkompilasifarmasi", norawat.replaceAll("/", ""));
        }
    }

    // ============================================================
    // METHOD UPDATE TOTAL PASIEN
    // ============================================================
    private void updateTotalPasien() {
