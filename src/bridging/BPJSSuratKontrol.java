...
MnKirimWAPdf = new javax.swing.JMenuItem();
MnKirimWAPdf.setBackground(new java.awt.Color(255, 255, 254));
MnKirimWAPdf.setFont(new java.awt.Font("Tahoma", 0, 11));
MnKirimWAPdf.setForeground(new java.awt.Color(50, 50, 50));
MnKirimWAPdf.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/whatsapp.png")));
MnKirimWAPdf.setText("Kirim WA Surat Kontrol");
MnKirimWAPdf.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
MnKirimWAPdf.setHorizontalTextPosition(javax.swing.SwingConstants.RIGHT);
MnKirimWAPdf.setName("MnKirimWAPdf");
MnKirimWAPdf.setPreferredSize(new java.awt.Dimension(180, 26));
MnKirimWAPdf.addActionListener(new java.awt.event.ActionListener() {
    public void actionPerformed(java.awt.event.ActionEvent evt) {
        MnKirimWAPdfActionPerformed(evt);
    }
});
jPopupMenu1.add(MnKirimWAPdf);
...
private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(NoRawat.getText().trim().equals("")||NoSEP.getText().trim().equals("")){
            ...
            // ========== VALIDASI TANGGAL KONTROL ==========
            String tglRencanaKontrol = Valid.SetTgl(TanggalKontrol.getSelectedItem()+"");
            String tglHariIni = new java.text.SimpleDateFormat("yyyy-MM-dd").format(new java.util.Date());
            
            if(tglRencanaKontrol.equals(tglHariIni)){
                JOptionPane.showMessageDialog(null, 
                    "Tanggal rencana kontrol tidak boleh sama dengan tanggal berobat. Rencana kontrol wajib dijadwalkan pada hari berikutnya",
                    "Validasi Tanggal Kontrol",
...
private void ChkInputActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ChkInputActionPerformed
  isForm();                
   ...
   private void MnKirimWAPdfActionPerformed(java.awt.event.ActionEvent evt) {
    if (tbObat.getSelectedRow() == -1) {
        JOptionPane.showMessageDialog(null, "Silakan pilih data surat kontrol terlebih dahulu!");
        return;
    }
    
    try {
        String noSurat = NoSurat.getText().trim();
        String noRawat = NoRawat.getText().trim();
        String noRM = NoRM.getText().trim();
        String namaPasien = NmPasien.getText().trim();
        String tglKontrol = TanggalKontrol.getSelectedItem().toString();
        String poli = NmPoli.getText().trim();
        String dokter = NmDokter.getText().trim();
        
        if (noSurat.isEmpty()) {
            JOptionPane.showMessageDialog(null, "Nomor surat tidak ditemukan!");
            return;
        }
        ...
        try {
            ...
            System.out.println("Error ambil nomor WA: " + e.getMessage());
        }
        
        if (nomorWA == null || nomorWA.isEmpty()) {
            nomorWA = JOptionPane.showInputDialog(this, "Masukkan nomor WhatsApp (contoh: 628123456789):");
            if (nomorWA == null || nomorWA.trim().isEmpty()) return;
            nomorWA = nomorWA.trim().replaceAll("[^0-9]", "");
            if (nomorWA.startsWith("0")) nomorWA = "62" + nomorWA.substring(1);
        }
        
        // ============= KONFIRMASI PREVIEW =============
        String previewPesan = "Yth. Bapak/Ibu " + namaPasien + "\n\n" +
                              "Dengan hormat,\n" +
                              "Kami menyampaikan Surat Kontrol BPJS dengan rincian:\n\n" +
                              "No. Surat Kontrol : " + noSurat + "\n" +
                              "Tanggal Kontrol   : " + tglKontrol + "\n" +
                              "Poli              : " + poli + "\n" +
                              "Dokter            : " + dokter + "\n\n" +
                              "PDF Surat Kontrol akan dilampirkan.\n\n" +
                              "Hormat kami,\n" +
                              akses.getnamars();
        
        int konfirmasi = JOptionPane.showConfirmDialog(this,
            "Preview Pesan WhatsApp:\n\n" + previewPesan + 
            "\n\nNomor Tujuan: " + formatNomorWA(nomorWA) +
        ...
        File pdfFile = cekDanBuatPDF(noSurat, noRawat);
        if (pdfFile == null) return;
        
        kirimWAPdf(nomorWA, pdfFile, namaPasien, noSurat, tglKontrol, poli, dokter);
    ...
    */
    ...
    private javax.swing.JMenuItem MnKirimWAPdf;
    ...
    private File cekDanBuatPDF(String noSurat, String noRawat) {
    File dir = new File("surat_kontrol");
    if (!dir.exists()) dir.mkdirs();
    
    File pdfFile = new File(dir, "SK_" + noSurat + ".pdf");
    ...
    try {
        ...
        param.put("finger", "Dikeluarkan di " + akses.getnamars() + 
                    ...
                    NmDokter.getText() + 
                    "\nID " + noSurat + 
                    "\n" + Valid.SetTgl3(TanggalKontrol.getSelectedItem() + ""));
        param.put("no_surat", noSurat);
        
        net.sf.jasperreports.engine.JasperPrint jp = 
            net.sf.jasperreports.engine.JasperFillManager.fillReport(
                "report/rptBridgingSuratKontrol2.jasper", param, koneksi);
        
        net.sf.jasperreports.engine.JasperExportManager.exportReportToPdfFile(jp, pdfFile.getAbsolutePath());
        
        JOptionPane.showMessageDialog(null, "PDF Surat Kontrol berhasil dibuat!");
        
    } catch (Exception e) {
        System.out.println("Error membuat PDF: " + e);
...
private void kirimWAPdf(String nomorWA, File pdfFile, String namaPasien, String noSurat, String tglKontrol, String poli, String dokter) {
    try {
        this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        
        String pesan = "Yth. Bapak/Ibu " + namaPasien + "\n\n" +
               "Dengan hormat,\n" +
               "Kami menyampaikan Surat Kontrol BPJS dengan rincian sebagai berikut:\n\n" +
               "No. Surat Kontrol : " + noSurat + "\n" +
               "Tanggal Kontrol   : " + tglKontrol + "\n" +
               "Poli              : " + poli + "\n" +
               "Dokter            : " + dokter + "\n\n" +
               "Mohon diperhatikan bahwa pasien diharapkan datang sesuai dengan tanggal yang tercantum pada Surat Kontrol. " +
               "Pasien tidak diperkenankan datang sebelum tanggal kontrol yang telah ditetapkan karena pelayanan hanya dapat diberikan sesuai jadwal yang tertera pada Surat Kontrol.\n\n" +
               "PDF Surat Kontrol terlampir.\n\n" +
               "Terima kasih.\n\n" +
               "Hormat kami,\n" +
               akses.getnamars();
        ...
        org.apache.http.client.methods.HttpPost httpPost = 
            new org.apache.http.client.methods.HttpPost("http://192.168.1.141:3000/send-document");
        ...
        org.apache.http.entity.mime.MultipartEntity multipart = new org.apache.http.entity.mime.MultipartEntity();
        multipart.addPart("phone", new org.apache.http.entity.mime.content.StringBody(nomorWA));
        multipart.addPart("caption", new org.apache.http.entity.mime.content.StringBody(pesan));
        multipart.addPart("document", new org.apache.http.entity.mime.content.ByteArrayBody(pdfBytes, "application/pdf", pdfFile.getName()));
        ...
        if (result.contains("success")) {
            JOptionPane.showMessageDialog(null, "✓ Berhasil dikirim ke " + nomorWA);
        } else {
            JOptionPane.showMessageDialog(null, "Gagal mengirim: " + result);
        }
...
private String formatNomorWA(String nomor) {
    ...
    String clean = nomor.replaceAll("[^0-9]", "");
    if (clean.length() >= 12) {
        return clean.substring(0, 2) + "-" + clean.substring(2, 5) + "-" + clean.substring(5, 9) + "-" + clean.substring(9);
    }
    return nomor;
}
